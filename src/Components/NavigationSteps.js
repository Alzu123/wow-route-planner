...
const beautifyNavigation = (node) => {
  return `${node.type} - ${node.origin.description} to ${node.destination.description}`
}
...
const NavigationSteps = ({ nodes, finalRoute }) => {
  ...
  return (
    <div>
      ...
      <ol>
        {finalRoute.map((node, index) =>
          index === 0 || index === finalRoute.length - 1 ? '' : <li key={index}>{beautifyNavigation(node)}</li>
        )}
